(function($) {
	...
	$.fn.build 				= function($this,o,mode,current,incfactor){
			...
			var $nav = $('<div/>',{
			    'class'	:	'nav',
				html	:	'<a class="thumbview grid"></a>' + autoplayHTML + '<div class="fsize11 floatL w100 padT5 padL5">'+o.imgCount +" "+ o.headerText+'</div><button class="next"></button><button class="prev '+prevClass+'"></button>'
			});
			if(!o.menu){
				$nav.css('top','0px');
				$this.hover(
			...
			var descFirst 	 = $this.attr('title').split('|',2);
			var descName 	 = descFirst[0];
			var descDate 	 = descFirst[1];
			
			var descHTML	 = '<div class=""><div style="float:left; font-size:12px; width:176px; padding-top:3px; font-weight:bold;">' + descName + '</div><div style="float:left; width:16px; margin-top:2px"><input type="button" value="" class="btnAdd"/></div><div style="font-style:italic; color:#e7e7c5; padding-top:0">' + descDate + '</div></div>';
			...
			/*$description.find("input.btnAdd").off().on({
				click: function(e){
