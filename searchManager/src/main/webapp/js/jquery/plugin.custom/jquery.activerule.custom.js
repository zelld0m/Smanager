(function($){
	...
	$.activerule = function(el, options){
		...
		base.getData = function(){
			...
			$.getJSON(
					GLOBAL_solrUrl.replace(configURL,currentURL) + "select?rows=0&wt=json&json.nl=map&gui=" + $.parseJSON(GLOBAL_solrConfig)["isFmGui"] + "&" + GLOBAL_solrSelectorParam + "=" + GLOBAL_storeId + "&q=" + base.options.keyword,
