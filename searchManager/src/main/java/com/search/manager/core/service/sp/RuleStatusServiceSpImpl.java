...
public class RuleStatusServiceSpImpl implements RuleStatusService {
    ...
    @Override
    public List<RuleStatus> add(Collection<RuleStatus> models) throws CoreServiceException {
        try {
            // TODO validation here...
    ...
    @Override
    public List<RuleStatus> update(Collection<RuleStatus> models) throws CoreServiceException {
        try {
            // TODO validation here...
    ...
    @Override
    public Map<RuleStatus, Boolean> delete(Collection<RuleStatus> models) throws CoreServiceException {
        try {
            // TODO validation here...
    ...
    @Override
    public SearchResult<RuleStatus> search(Search search) throws CoreServiceException {
        try {
    ...
    @Override
    public SearchResult<RuleStatus> search(RuleStatus model) throws CoreServiceException {
        if (model != null) {
            try {
    ...
    @Override
    public SearchResult<RuleStatus> search(RuleStatus model, int pageNumber, int maxRowCount)
    ...
    @Override
    public RuleStatus searchById(String storeId, String id) throws CoreServiceException {
        if (StringUtils.isNotBlank(storeId) && StringUtils.isNotBlank(id)) {
            ...
            if (searchResult.getTotalCount() > 0) {
                return (RuleStatus) CollectionUtils.get(searchResult.getResult(), 0);
            }
        }
    ...
    @Override
    public RuleStatus updateRuleStatusExportInfo(RuleStatus ruleStatus, String exportBy, ExportType exportType,
    ...
    @Override
    public RuleStatus updateRuleStatusPublishInfo(RuleStatus ruleStatus, RuleStatusEntity requestedPublishStatus,
            String requestBy, DateTime requestDateTime) throws CoreServiceException {
        // TODO validation here...
        RuleStatus updateRuleStatus = getRuleStatusPK(ruleStatus);
    ...
    @Override
    public RuleStatus updateRuleStatusApprovalInfo(RuleStatus ruleStatus, RuleStatusEntity requestedApprovalStatus,
            String requestBy, DateTime requestDateTime) throws CoreServiceException {
        try {
            if (requestedApprovalStatus != null) {
                ...
                SearchResult<RuleStatus> searchResult = search(ruleStatusFilter, 1, 1);

                if (searchResult.getTotalCount() > 0) {
                    // existing rule
                    RuleStatus existingRuleStatus = searchResult.getResult().get(0);

                    // if rule is not for deletion do not change update status.
                    if (!StringUtils.equalsIgnoreCase(existingRuleStatus.getUpdateStatus(),
                            String.valueOf(RuleStatusEntity.DELETE))
                            && (StringUtils.isBlank(existingRuleStatus.getUpdateStatus()) || StringUtils
                                    .equalsIgnoreCase(existingRuleStatus.getPublishedStatus(),
    ...
    @Override
    public boolean updateRuleStatusDeletedInfo(RuleStatus ruleStatus, String deletedBy) throws CoreServiceException {
        // TODO validation here...
        RuleStatus ruleStatusFilter = new RuleStatus();
        ruleStatusFilter.setRuleTypeId(ruleStatus.getRuleTypeId());
        ruleStatusFilter.setStoreId(ruleStatus.getStoreId());
        ...
        if (searchResult.getTotalCount() > 0) {
            RuleStatus updateRuleStatus = getRuleStatusPK(ruleStatus);
            updateRuleStatus.setApprovalStatus(StringUtils.equalsIgnoreCase(searchResult.getResult().get(0)
                    .getPublishedStatus(), String.valueOf(RuleStatusEntity.UNPUBLISHED)) ? "" : String
    ...
    @Override
    public RuleStatus getRuleStatus(String ruleType, String ruleRefId) throws CoreServiceException {
        if (StringUtils.isNotBlank(ruleType) && StringUtils.isNotBlank(ruleRefId)) {
            RuleStatus ruleStatus = new RuleStatus();
