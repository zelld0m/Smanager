...
public class AuditTrailDaoSpImpl extends GenericDaoSpImpl<AuditTrail> implements AuditTrailDao {
    ...
    @Override
    protected Map<String, Object> generateAddInput(AuditTrail model) throws CoreDaoException {
        ...
        if (model != null) {
            ...
            inputs.put(DAOConstants.PARAM_DATE, jodaDateTimeUtil.toSqlDate(model.getCreatedDate()));
    ...
    @Override
    protected Map<String, Object> generateUpdateInput(AuditTrail model) throws CoreDaoException {
        throw new CoreDaoException("Unsupported Operation.");
    }

    @Override
    protected Map<String, Object> generateDeleteInput(AuditTrail model) throws CoreDaoException {
        throw new CoreDaoException("Unsupported Operation.");
    }

    @Override
    protected Search generateSearchInput(AuditTrail model) throws CoreDaoException {

        if (model != null) {
            Search search = new Search(AuditTrail.class);

            if (StringUtils.isNotBlank(model.getUsername())) {
                search.addFilter(new Filter(DAOConstants.PARAM_USER_NAME, model.getUsername()));
            }
            if (StringUtils.isNotBlank(model.getOperation())) {
                search.addFilter(new Filter(DAOConstants.PARAM_OPERATION, model.getOperation()));
            }
            if (StringUtils.isNotBlank(model.getEntity())) {
                search.addFilter(new Filter(DAOConstants.PARAM_ENTITY, model.getEntity()));
            }
            if (StringUtils.isNotBlank(model.getStoreId())) {
                search.addFilter(new Filter(DAOConstants.PARAM_STORE, model.getStoreId()));
            }
            if (StringUtils.isNotBlank(model.getKeyword())) {
                search.addFilter(new Filter(DAOConstants.PARAM_KEYWORD, model.getKeyword()));
            }
            if (StringUtils.isNotBlank(model.getReferenceId())) {
                search.addFilter(new Filter(DAOConstants.PARAM_REFERENCE, model.getReferenceId()));
            }
            if (model.getCreatedDate() != null) {
                search.addFilter(new Filter(DAOConstants.PARAM_DATE, jodaDateTimeUtil.toSqlDate(model.getCreatedDate())));
            }
            if (StringUtils.isNotBlank(model.getDetails())) {
                search.addFilter(new Filter(DAOConstants.PARAM_DETAILS, model.getDetails()));
            }
            return search;
        }
    ...
    @Override
    public List<String> getRefIDs(String entity, String operation, String storeId) {
        String sql = GET_REFID_SQL;

        return searchSp.getJdbcTemplate().query(sql, new String[] {
            entity, operation, storeId
