...
public class ImagePathDaoSpImpl extends GenericDaoSpImpl<ImagePath> implements ImagePathDao {
    ...
    @Override
    protected Map<String, Object> generateAddInput(ImagePath model) throws CoreDaoException {
    ...
    @Override
    protected Map<String, Object> generateUpdateInput(ImagePath model) throws CoreDaoException {
        Map<String, Object> inputs = null;
    ...
    @Override
    protected Map<String, Object> generateDeleteInput(ImagePath model) throws CoreDaoException {
        throw new CoreDaoException("Unsupported Operation.");
    }

    @Override
    protected Search generateSearchInput(ImagePath model) throws CoreDaoException {
        if (model != null) {
            Search search = new Search(ImagePath.class);

            if (StringUtils.isNotBlank(model.getId())) {
                search.addFilter(new Filter(DAOConstants.PARAM_IMAGE_PATH_ID, model.getId()));
            }
            ...
            if (StringUtils.isNotBlank(model.getPath())) {
                search.addFilter(new Filter(DAOConstants.PARAM_IMAGE_PATH, model.getPath()));
            }
            if (StringUtils.isNotBlank(model.getSize())) {
                search.addFilter(new Filter(DAOConstants.PARAM_IMAGE_SIZE, model.getSize()));
            }
            if (model.getPathType() != null) {
                search.addFilter(new Filter(DAOConstants.PARAM_IMAGE_PATH_TYPE, model.getPathType().toString()));
            }
            if (StringUtils.isNotBlank(model.getAlias())) {
                search.addFilter(new Filter(DAOConstants.PARAM_IMAGE_PATH_ALIAS, model.getAlias()));
            }
            return search;
        }
        return null;
