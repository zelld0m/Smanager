...
public class SolrSearchProcessor extends BaseSearchProcessor implements SearchProcessor {
    ...
    public SearchResult<?> processSearch(Search search) throws CoreSearchException {
        ...
        try {
            ...
            if (solrCore != null) {
                ...
                solrServer = solrServerFactory.getCoreInstance(solrCore.name()).getSolrServer();
                ...
                if (solrServer != null) {
                    ...
                    if (fields != null && fields.size() > 0) {
                        for (Field field : fields) {
                            if (field.getOperator() != FieldOperator.PROPERTY) {
                                throw new Exception("Field operation not supported in solr.");
                            }
                    ...
                    if (StringUtils.isNotEmpty(where)) {
                        solrQuery.addFilterQuery(where.substring(3, where.length()));
                    }

                    // sort
                    List<Sort> sorts = checkAndCleanSorts(search.getSorts());
                    for (Sort sort : sorts) {
                        solrQuery.addSort(sort.getProperty(), sort.isDesc() ? ORDER.desc : ORDER.asc);
                    }
                    ...
                    if (queryResponse != null && queryResponse.getStatus() == 0) {
                        searchResult.setResult(queryResponse.getBeans(search.getSearchClass()));
                        searchResult.setTotalCount(queryResponse.getBeans(search.getSearchClass()).size());
                        return searchResult;
                    }
    ...
    // Select Clause
    @Override
    protected String generateSelectClause(Search search) throws CoreSearchException {
        List<Field> fields = checkAndCleanFields(search.getFields());
        ...
        if (fields != null && fields.size() > 0) {
            for (Field field : fields) {
                if (field.getOperator() != FieldOperator.PROPERTY) {
                    throw new CoreSearchException("Field operation not supported in solr.");
                }
                if (first) {
    ...
    // Where Clause
    @Override
    protected String generateWhereClause(Search search) throws CoreSearchException {
        ...
        if (filters == null || filters.size() == 0) {
            ...
            Filter junction = new Filter(null, filters, isDisjunction ? FilterOperator.OR : FilterOperator.AND);
    ...
    // Order By Clause
    @Override
    protected String generateOrderByClause(Search search) throws CoreSearchException {
        List<Sort> sorts = checkAndCleanSorts(search.getSorts());
    ...
    protected String toFilterQuery(Filter filter) {
        ...
        switch (operator) {
            ...
            case OR:
                ...
                String op = filter.getOperator() == FilterOperator.AND ? " AND " : " OR ";
    ...
    protected String param(Object value) {
        ...
        if (value instanceof DateTime) {
            ...
            return "\"" + ((DateTime) value).withZone(DateTimeZone.UTC).toString("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") + "\"";
    ...
    @Override
    protected List<Field> checkAndCleanFields(List<Field> fields) throws CoreSearchException {
        // TODO Auto-generated method stub
        return fields;
    }

    @Override
    protected List<Filter> checkAndCleanFilters(List<Filter> filters) throws CoreSearchException {
        // TODO Auto-generated method stub
        return filters;
    }

    @Override
    protected List<Sort> checkAndCleanSorts(List<Sort> sorts) throws CoreSearchException {
        // TODO Auto-generated method stub
        return sorts;
    }
