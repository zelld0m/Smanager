...
@Repository("importRuleTaskDaoSp")
public class ImportRuleTaskDaoSpImpl extends GenericDaoSpImpl<ImportRuleTask> implements ImportRuleTaskDao {
    ...
    @Override
    protected Map<String, Object> generateAddInput(ImportRuleTask model) throws CoreDaoException {
    ...
    @Override
    protected Map<String, Object> generateUpdateInput(ImportRuleTask model) throws CoreDaoException {
        Map<String, Object> inputs = null;
        ...
        if (model != null) {
            ...
            inputs.put(
                    WorkflowConstants.COLUMN_STATE_COMPLETED,
                    taskExecutionResult.getStateCompleted() != null ? taskExecutionResult.getStateCompleted().ordinal() + 1
                            : 0);
            inputs.put(WorkflowConstants.COLUMN_TASK_START_STAMP,
                    jodaDateTimeUtil.toSqlDate(taskExecutionResult.getTaskStartDateTime()));
            inputs.put(WorkflowConstants.COLUMN_TASK_END_STAMP,
                    jodaDateTimeUtil.toSqlDate(taskExecutionResult.getTaskEndDateTime()));
            inputs.put(WorkflowConstants.COLUMN_LAST_UPDATED_BY, model.getLastModifiedBy());
            inputs.put(WorkflowConstants.COLUMN_LAST_UPDATED_STAMP,
                    jodaDateTimeUtil.toSqlDate(model.getLastModifiedDate()));
        }
        return inputs;
    }

    @Override
    protected Map<String, Object> generateDeleteInput(ImportRuleTask model) throws CoreDaoException {
        // TODO Auto-generated method stub
        return null;
    }

    @Override
    protected Search generateSearchInput(ImportRuleTask model) throws CoreDaoException {
        Search search = new Search(model.getClass());
        List<Filter> filters = new ArrayList<Filter>();
        if (model != null) {
            TaskExecutionResult taskExecutionResult = model.getTaskExecutionResult();

            filters.add(new Filter(WorkflowConstants.COLUMN_RULE_TYPE_ID, model.getRuleEntity() != null ? model
                    .getRuleEntity().getCode() : null));
            ...
            filters.add(new Filter(WorkflowConstants.COLUMN_IMPORT_TYPE, model.getImportType() != null ? model
                    .getImportType().ordinal() + 1 : null));
            filters.add(new Filter(WorkflowConstants.COLUMN_TASK_STATUS,
                    taskExecutionResult != null ? taskExecutionResult.getTaskStatus().ordinal() + 1 : null));
            filters.add(new Filter(
                    WorkflowConstants.COLUMN_TASK_START_STAMP,
                    taskExecutionResult != null ? jodaDateTimeUtil.toSqlDate(taskExecutionResult.getTaskStartDateTime())
                            : null));
            filters.add(new Filter(WorkflowConstants.COLUMN_TASK_END_STAMP,
                    taskExecutionResult != null ? jodaDateTimeUtil.toSqlDate(taskExecutionResult.getTaskEndDateTime())
                            : null));
            filters.add(new Filter(WorkflowConstants.COLUMN_CREATED_STAMP, jodaDateTimeUtil.toSqlDate(model
                    .getCreatedDate())));
