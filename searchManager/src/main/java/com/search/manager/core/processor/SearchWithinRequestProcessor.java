...
public class SearchWithinRequestProcessor implements RequestProcessor {
    ...
    public boolean isEnabled(RequestPropertyBean requestPropertyBean) {
        return BooleanUtils.toBooleanObject(StringUtils.defaultIfBlank(configManager.getProperty(PROPERTY_MODULE_NAME,
                requestPropertyBean.getStoreId(), "searchwithin.enable"), "false"));
    }
    ...
    public void process(HttpServletRequest request, SolrResponseParser solrHelper,
        ...
        if (logger.isInfoEnabled()) {
            logger.info(String.format("%s Enabled? %s, Param exists? %s", getRequestParamName(requestPropertyBean),
                    BooleanUtils.toStringYesNo(isEnabled(requestPropertyBean)),
                    BooleanUtils.toStringYesNo(ArrayUtils.getLength(paramValues) > 0)));
            if (ArrayUtils.getLength(paramValues) > 0) {
                logger.info(String.format("Param count? %s Value read: %s", ArrayUtils.getLength(paramValues),
                        StringUtils.join(paramValues, "|")));
            }
        }

        if (!isEnabled(requestPropertyBean) || ArrayUtils.getLength(paramValues) == 0
                || StringUtils.isBlank(swValues = paramValues[paramValues.length - 1])) {
            logger.info("Skipped: Enabled? {}, Empty params? {}",
                    BooleanUtils.toStringYesNo(isEnabled(requestPropertyBean)),
                    BooleanUtils.toStringYesNo(StringUtils.isBlank(swValues)));
            return;
        }

        if (CollectionUtils.isEmpty(solrFieldToSearchList = getFields(requestPropertyBean))
                || CollectionUtils.isEmpty(swTypeList = getSearchWithinType(requestPropertyBean))) {
            logger.info("Skipped: Empty search fields? {} , Empty allowed params? {}",
                    BooleanUtils.toStringYesNo(CollectionUtils.isEmpty(solrFieldToSearchList)),
                    BooleanUtils.toStringYesNo(CollectionUtils.isEmpty(getSearchWithinType(requestPropertyBean))));
            return;
        }
        ...
        try {
            ...
            for (String swType : swTypeList) {
                if (StringUtils.isNotBlank(swType)) {
                    ...
                    if (swJSONParam.containsKey(swType)
                            && !JSONUtils.isNull(jsonArray = swJSONParam.getJSONArray(swType))) {
                        swParamList = (List<String>) JSONSerializer.toJava(jsonArray);
                    }

                    List<String> tokensList = new ArrayList<String>();
                    if (CollectionUtils.isNotEmpty(swParamList)
                            && CollectionUtils.isNotEmpty(tokensList = getTokenizedKeyword(requestPropertyBean,
                                    swParamList, swType))) {
    ...
    public List<String> getFields(RequestPropertyBean requestPropertyBean) {
        return configManager.getPropertyList(PROPERTY_MODULE_NAME, requestPropertyBean.getStoreId(),
                "searchwithin.solrfieldlist");
    }
    ...
    public String getRequestParamName(RequestPropertyBean requestPropertyBean) {
        return StringUtils.defaultIfBlank(configManager.getProperty(PROPERTY_MODULE_NAME,
                requestPropertyBean.getStoreId(), "searchwithin.paramname"), "searchWithin");
    }

    public String getKeywordOperator(RequestPropertyBean requestPropertyBean, String swType) {
        return StringUtils.defaultIfBlank(
                configManager.getProperty(PROPERTY_MODULE_NAME, requestPropertyBean.getStoreId(),
                        String.format("searchwithin.%s.keywordOperator", swType)), "OR");
    }

    public String getSolrFieldOperator(RequestPropertyBean requestPropertyBean, String swType) {
        return StringUtils.defaultIfBlank(
                configManager.getProperty(PROPERTY_MODULE_NAME, requestPropertyBean.getStoreId(),
                        String.format("searchwithin.%s.solrFieldOperator", swType)), "OR");
    }

    public String getPrefixOperator(RequestPropertyBean requestPropertyBean, String swType) {
        return configManager.getProperty(PROPERTY_MODULE_NAME, requestPropertyBean.getStoreId(),
                String.format("searchwithin.%s.prefixTypeOperator", swType));
    }

    public boolean isQuoteKeyword(RequestPropertyBean requestPropertyBean, String swType) {
        return BooleanUtils.toBooleanObject(StringUtils.defaultIfBlank(
                configManager.getProperty(PROPERTY_MODULE_NAME, requestPropertyBean.getStoreId(),
                        String.format("searchwithin.%s.quoteKeyword", swType)), "false"));
    }

    public boolean isSplit(RequestPropertyBean requestPropertyBean, String swType) {
        return BooleanUtils.toBooleanObject(StringUtils.defaultIfBlank(
                configManager.getProperty(PROPERTY_MODULE_NAME, requestPropertyBean.getStoreId(),
                        String.format("searchwithin.%s.split", swType)), "false"));
    }

    public String getSplitRegex(RequestPropertyBean requestPropertyBean, String swType) {
        return configManager.getProperty(PROPERTY_MODULE_NAME, requestPropertyBean.getStoreId(),
                String.format("searchwithin.%s.splitRegex", swType));
    }

    public int getMinLength(RequestPropertyBean requestPropertyBean) {
        return Integer.parseInt(StringUtils.defaultIfBlank(configManager.getProperty(PROPERTY_MODULE_NAME,
                requestPropertyBean.getStoreId(), "searchwithin.minLength"), "2"));
    }

    public List<String> getTokenizedKeyword(RequestPropertyBean requestPropertyBean, List<String> keywords,
            String allowedSwParam) {
        ...
        for (String keyword : keywords) {
            ...
            if (StringUtils.isNotBlank(trimmedKeyword)) {
                ...
                if (numberOfQuotes > 0 && (numberOfQuotes % 2) == 0 && StringUtils.startsWith(trimmedKeyword, "\"")
                        && StringUtils.endsWith(trimmedKeyword, "\"")) {
                ...
                logger.info("Tokens using {}: {}", getSplitRegex(requestPropertyBean, allowedSwParam),
    ...
    public String[] toSolrFq(RequestPropertyBean requestPropertyBean, Map<String, List<String>> swProcessedParams)
            throws Throwable {
        ...
        if (CollectionUtils.isNotEmpty(fields) && MapUtils.isNotEmpty(swProcessedParams)) {
            ...
            try {
                for (String swType : keySet) {
                    ...
                    swKeywordTemplate = String.format(
                            CollectionUtils.size(fields) == 1
                                    || CollectionUtils.size(swProcessedParams.get(swType)) == 1 ? "%s" : "(%s)",
                            StringUtils.join(fields, ":%%keyword%% %%operator%% ") + ":%%keyword%%");
                    String keywordTemplate = isQuoteKeyword(requestPropertyBean, swType) ? "\"%s\"" : "%s";
                    for (String swKeyword : swProcessedParams.get(swType)) {
                        sbType.append(sbType.length() > 0 ? String.format(" %s ",
                                getKeywordOperator(requestPropertyBean, swType)) : "");
                        sbType.append(StringUtils.replaceEach(swKeywordTemplate, new String[] {
                            "%%keyword%%", "%%operator%%"
                        }, new String[] {
                            String.format(keywordTemplate, StringEscapeUtils.escapeJavaScript(swKeyword)),
                            getSolrFieldOperator(requestPropertyBean, swType)
                        }));
                    }

                    String prefixOperator = getPrefixOperator(requestPropertyBean, swType);
                    String perTypeTemplate = String.format("(%s)",
                            StringUtils.isNotBlank(prefixOperator) ? StringUtils.trim(prefixOperator) + "(%s)" : "%s");

                    perTypeQueryArr[iteration++] = String.format(perTypeTemplate, sbType.toString());
                }

                if (logger.isInfoEnabled()) {
                    logger.info("Generated Solr Filter: fq={}",
                            StringUtils.join(perTypeQueryArr, String.format("%s", "&fq=")));
                }

                if (ArrayUtils.isNotEmpty(perTypeQueryArr))
                    return perTypeQueryArr;
            } catch (Exception e) {
                throw e;
            } catch (Throwable t) {
                throw t;
