...
public class BoostQueryModel implements VerifiableModel {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(BoostQueryModel.class);
    ...
    public boolean getIsManufacturerOnly() {
        ...
        for (BoostQuery boostQuery : expression) {
            try {
                ...
                if (!StringUtils.equalsIgnoreCase(sq.getField().getName(), "Manufacturer")) {
                    return false;
                }
            } catch (Exception e) {
                return false;
    ...
    public boolean getIsCategoryOnly() {
        ...
        for (BoostQuery boostQuery : expression) {
            try {
                Expression<SubQuery, SubQuery> expression = boostQuery.getExpression();
                SubQuery sq = expression.getLValue();
                if (!StringUtils.equalsIgnoreCase(sq.getField().getName(), "Category")) {
                    return false;
                }
            } catch (Exception e) {
                return false;
