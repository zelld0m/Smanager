...
public final class XSSUtil {
	...
	/**
	 * this class is a wrapper to prevent concurrency problem when using non-threadsafe Pattern.matcher 
	 * 
	 * @author genarop
	 *
	 */
	...
	public final static String stripBlackListedTags(String input) {
		...
		for (String bl : HTMLPATTERN_LIST.BLACKLISTED_TAGS.getList()) {
			input = input.replaceAll(bl, " "); /** put space instead of blank to prevent other xss attack style **/
		}
		return input;
	}
	...
	public final static String stripWatchListStrOnEachTag(String input) {
		...
		while(m.find()) {
			...
			for (String bl : HTMLPATTERN_LIST.ATTRIB_CMD_WATCHLIST.getList()) {
				singleNodeTag = singleNodeTag.replaceAll(bl, " "); /** put space instead of blank to prevent other xss attack style **/
			}
	...
	/**
	 * Strip off Unicode characters. These unicode chars can do harmful tricks when applied inside tags 
	 * 
	 * @param input
	...
	public final static String stripPeskyUnicodedChars(String input) {
		if (!StringUtil.isBlank(input)) {
			...
			input = input.replaceAll("(&#([xX]?)[0-9a-zA-Z]{1,7})", " "); /** put space instead of blank to prevent other xss attack style **/
			/** remove - \\u[ALPHANUMERIC], %[ALPHANUMERIC]  **/
			input = input.replaceAll("([\\x25\\x5C][uU]?[0-9a-zA-Z]{1,7})", " "); /** put space instead of blank to prevent other xss attack style **/
			return input;
		} else {
			return input;
		}
...
//		input = input.replaceAll("[\\x00-\\x1F\\x7F]", "");
	...
	public static void main(String[] args) {
		System.out.println(hasWatchListStrOnEachTag("<p >url</p>")); 
		System.out.println(stripWatchListStrOnEachTag("<p style='url:'>u rl</p>")); 
	}
