...
public class BannerReportBean extends ReportBean<BannerRuleItem> {
    ...
    @ReportField(label = "Size", size = 15, sortOrder = 2)
    public String getImageSize() {
    	return model.getImagePath() != null ? StringUtils.defaultString(model.getImagePath().getSize()) : "";
    }
    
    @ReportField(label = "Alias", size = 15, sortOrder = 3)
    public String getAlias() {
        return model.getImagePath() != null ? StringUtils.defaultString(model.getImagePath().getAlias()) : "";
    }

    @ReportField(label = "Image Path", size = 20, sortOrder = 4)
    public String getImagePath() {
        return model.getImagePath() != null ? StringUtils.defaultString(model.getImagePath().getPath()) : "";
    }

    @ReportField(label = "Link Path", size = 20, sortOrder = 5)
    public String getLinkPath() {
        return StringUtils.defaultString(model.getLinkPath());
    }

    @ReportField(label = "Image Alt", size = 20, sortOrder = 6)
