...
public class BannerRuleItemMigratorService implements SolrRuleService<BannerRuleItem> {
    ...
    // create model
    private BannerRuleItem toBannerRuleItem(String storeId, String ruleId, String ruleName, String memberId)
    ...
    // search
    private SearchResult<BannerRuleItem> search(String storeId, String ruleId, String ruleName, String memberId)
            throws CoreServiceException {
        return bannerRuleItemServiceSolr.search(toBannerRuleItem(storeId, ruleId, ruleName, memberId));
    }

    @RemoteMethod
    @Override
    public SearchResult<BannerRuleItem> searchByStoreId(String storeId) throws CoreServiceException {
        return search(storeId, null, null, null);
    }

    @RemoteMethod
    @Override
    public BannerRuleItem searchByStoreKeyword(String storeId, String keyword) throws CoreServiceException {
        return searchByRuleName(storeId, keyword);
    }

    @RemoteMethod
    @Override
    public BannerRuleItem searchByRuleId(String storeId, String ruleId) throws CoreServiceException {
        SearchResult<BannerRuleItem> searchResult = search(storeId, ruleId, null, null);
    ...
    @RemoteMethod
    @Override
    public BannerRuleItem searchByRuleName(String storeId, String ruleName) throws CoreServiceException {
        SearchResult<BannerRuleItem> searchResult = search(storeId, null, ruleName, null);
    ...
    @RemoteMethod
    @Override
    public BannerRuleItem searchByMemberId(String storeId, String memberId) throws CoreServiceException {
        SearchResult<BannerRuleItem> searchResult = search(storeId, null, null, memberId);
    ...
    // load
    private boolean load(String storeId, String ruleId, String ruleName, String memberId) throws CoreServiceException {
        int page = 1;
        BannerRuleItem bannerRuleItem = toBannerRuleItem(storeId, ruleId, ruleName, memberId);
        while (true) {
            SearchResult<BannerRuleItem> searchResult = bannerRuleItemServiceSp.search(bannerRuleItem, page, MAX_ROWS);
    ...
    private Map<String, Boolean> delete(List<BannerRuleItem> models, String modelField) throws CoreServiceException {
        Map<BannerRuleItem, Boolean> status = bannerRuleItemServiceSolr.delete(models);

        if (status != null) {
            Map<String, Boolean> deleteStatus = new HashMap<String, Boolean>();
            Iterator<Entry<BannerRuleItem, Boolean>> it = status.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry<BannerRuleItem, Boolean> entry = it.next();
                if (modelField.equals("ruleId") && entry.getKey().getRule() != null) {
                    deleteStatus.put(entry.getKey().getRule().getRuleId(), entry.getValue());
                } else if ((modelField.equals("keyword") || modelField.equals("ruleName"))
                        && entry.getKey().getRule() != null) {
                    deleteStatus.put(entry.getKey().getRule().getRuleName(), entry.getValue());
                } else if (modelField.equals("memberId")) {
                    deleteStatus.put(entry.getKey().getMemberId(), entry.getValue());
                }
            }
            return deleteStatus;
        }
    ...
    public Map<String, Boolean> deleteByStoreKeywords(String storeId, Collection<String> keywords)
        ...
        if (keywords != null) {
            ...
            for (String keyword : keywords) {
                bannerRuleItems.add(toBannerRuleItem(storeId, null, keyword, null));
            }
    ...
    public Map<String, Boolean> deleteByRuleIds(String storeId, Collection<String> ruleIds) throws CoreServiceException {
        if (ruleIds != null) {
            List<BannerRuleItem> bannerRuleItems = new ArrayList<BannerRuleItem>();
            for (String ruleId : ruleIds) {
                bannerRuleItems.add(toBannerRuleItem(storeId, ruleId, null, null));
            }
    ...
    public Map<String, Boolean> deleteByRuleNames(String storeId, Collection<String> ruleNames)
        ...
        if (ruleNames != null) {
            List<BannerRuleItem> bannerRuleItems = new ArrayList<BannerRuleItem>();
            for (String ruleName : ruleNames) {
                bannerRuleItems.add(toBannerRuleItem(storeId, null, ruleName, null));
            }
    ...
    public Map<String, Boolean> deleteByMemberIds(String storeId, Collection<String> memberIds)
        ...
        if (memberIds != null) {
            List<BannerRuleItem> bannerRuleItems = new ArrayList<BannerRuleItem>();
            for (String memberId : memberIds) {
                bannerRuleItems.add(toBannerRuleItem(storeId, null, null, memberId));
            }
