...
@Service("deploymentRuleServiceCacheImpl")
public class DeploymentRuleServiceCacheImpl implements DeploymentRuleService {

	private static Logger logger = Logger
			.getLogger(DeploymentRuleServiceCacheImpl.class);
		...
		try {
			...
			for (String key : list) {
				try {
					...
					excludeList = daoServiceStg.getExcludeResultList(criteria)
							.getList();
	...
	public Map<String, Boolean> publishRedirectRulesMap(String store,
		...
		try {
			...
			Store s = new Store(store);
			daoCacheService.reloadAllKeywords(s);
			daoCacheService.setForceReloadRedirect(s); // invalidates all cached
														// data to force a
														// reload and prevent
														// stale data
		} catch (Exception e) {
			...
			for (String key : list) {
				try {
					...
					if (addRel != null) {
						...
						Map<String, String> relevancyFields = addRel
								.getParameters();
					...
					Store s = new Store(store);
					daoCacheService.reloadAllKeywords(s);
					daoCacheService.setForceReloadRelevancy(s); // invalidates
																// all cached
																// data to force
																// a reload and
																// prevent stale
		...
		if (CollectionUtils.isNotEmpty(list)) {
			for (String key : list) {
				try {
					...
					daoService.clearElevateResult(new StoreKeyword(store, key)); // prod
					daoCacheService.resetElevateRule(sk); // clear sk entry in
															// cache
					daoCacheService.setForceReloadExclude(new Store(store));
					map.put(key, true);
				} catch (Exception e) {
		...
		if (CollectionUtils.isNotEmpty(list)) {
			for (String key : list) {
				try {
					...
					daoCacheService.resetExcludeRule(sk); // clear sk entry in
															// cache
					daoCacheService.setForceReloadExclude(new Store(store));
					map.put(key, true);
				} catch (Exception e) {
			...
			for (String key : list) {
				try {
					...
					SearchCriteria<RedirectRule> criteria = new SearchCriteria<RedirectRule>(
							rule, null, null, 0, 0);
					for (StoreKeyword keyword : daoService.getRedirectKeywords(
							criteria, MatchType.MATCH_ID, ExactMatch.SIMILAR)
							.getList()) {
						sks.add(keyword);
					}
					daoService.deleteRedirectRule(delRel); // prod
					for (StoreKeyword sk : sks) {
						daoCacheService.resetRedirectRule(sk); // clear sk entry
																// in cache
					}
					daoCacheService.setForceReloadRedirect(s); // invalidates
																// all cached
																// data to force
																// a reload and
																// prevent stale
			...
			for (String key : list) {
				try {
					...
					for (RelevancyKeyword keyword : daoService
							.getRelevancyKeywords(delRel).getList()) {
						sks.add(new StoreKeyword(store, keyword.getKeyword()
								.getKeywordId()));
					}
					daoService.deleteRelevancy(delRel); // prod
					for (StoreKeyword sk : sks) {
						daoCacheService.resetRelevancyRule(sk); // clear sk
																// entry in
																// cache
					}
					daoCacheService.setForceReloadRelevancy(s); // invalidates
		...
		if (CollectionUtils.isNotEmpty(list)) {
			for (String key : list) {
				try {
					...
					addEl.setStoreKeyword(sk);
					daoService.clearDemoteResult(new StoreKeyword(store, key)); // prod
					daoCacheService.resetDemoteRule(sk); // clear sk entry in
															// cache
					daoCacheService.setForceReloadDemote(new Store(store));
					map.put(key, true);
				} catch (Exception e) {
	...
	public Map<String, Boolean> publishFacetSortRulesMap(String store,
		...
		try {
				...
				if (addFs != null) {
					try {
						...
						SearchCriteria<FacetGroup> criteria = new SearchCriteria<FacetGroup>(
								facetGroup);
						RecordSet<FacetGroup> addFsGroups = daoServiceStg
			...
			Store s = new Store(store);
			daoCacheService.reloadAllKeywords(s);
			daoCacheService.setForceReloadFacetSort(s); // invalidates all
														// cached data to force
														// a reload and prevent
														// stale data
		} catch (Exception e) {
			logger.error(e, e);
		}
